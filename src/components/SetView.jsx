...
export default function SetView({ set, uid, go, toast }) {
  ...
  const [choosing, setChoosing] = useState(null); // 'learn' | 'listen' | null
  const c = counts(set);
  ...
  return (
    <>
      ...
      <div className="setHead">
        ...
        <div className="row">
          <button className="btn primary big" type="button" disabled={!set.cards.length} onClick={() => setChoosing(v => (v === 'learn' ? null : 'learn'))}>
            {allMastered ? 'Learn again' : c[1] + c[2] ? 'Continue learning' : 'Learn'}
          </button>
          <button className="btn listen-btn" type="button" disabled={!set.cards.length}
            onClick={() => setChoosing(v => (v === 'listen' ? null : 'listen'))}>
            <svg viewBox="0 0 24 24" width="16" height="16" aria-hidden="true"><path d="M4 13a8 8 0 0 1 16 0v5a2 2 0 0 1-2 2h-1v-7h3M4 13v5a2 2 0 0 0 2 2h1v-7H4" fill="none" stroke="currentColor" strokeWidth="2" strokeLinejoin="round" /></svg>
            Listen
          </button>
        ...
        {choosing && (
          <StudyOptions key={choosing} set={set} mode={choosing} onCancel={() => setChoosing(null)}
            onStart={cfg => {
              if (choosing === 'listen') { local.set(`recall.listen.${set.id}`, cfg); go('listen', set.id); }
              else learn(cfg);
            }} />
        )}
        {confirmDelete && (
